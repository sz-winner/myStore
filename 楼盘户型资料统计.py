...
while unit_loop:
    ...
    # 开始输入模板数据
    input('接下来请输入起始层每个房间的门牌号、南北朝向、面积、房间数、客厅及卫生间数量，按任意键继续')
    ...
    while room_loop:
        ...
        bedroom = int(input('请输入 %d 的卧室数量：' % room_number))
        parlor = int(input('请输入 %d 的客厅数量：' % room_number))
        bathroom = int(input('请输入 %d 的卫生间数量：' % room_number))
        start_floor_rooms[room_number] = [
            direction, area, bedroom, parlor, bathroom
        ]
    ...
    #unit_rooms={1:{101:[1,80],102:[1,80],103:[2,90],104:[2,90]}}
    ...
    with open('D:\\Python\\Property_test.csv', 'a', newline='') as csvfile:
